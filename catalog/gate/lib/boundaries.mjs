// boundaries.mjs — linter ESTÁTICO de fronteras de arquitectura (sin IA, sin tokens).
...
// Razón de cambio: las reglas de capas y la sintaxis de import por lenguaje.
//
// Vive en el portón porque es el portón quien lo ejecuta en cada tarea. `lib/verify-boundaries.mjs`
// lo re-exporta para `chalc verify`: una sola implementación, dos consumidores.
...
// Rutas importadas en el contenido CON su línea, en orden de aparición. La línea la necesita el
// portón de calidad, que reporta cada hallazgo como archivo:línea.
export function importsWithLines(content, ext) {
  const text = String(content || '');
  const found = [];
  const scan = (re) => {
    for (const m of text.matchAll(re)) found.push({ path: m[1], index: m.index });
  };

  if (ext === 'dart') {
    scan(/(?:import|export)\s+['"]([^'"]+)['"]/g);
  } else {
    scan(/(?:import|export)\s+(?:[^'"]*?\s+from\s+)?['"]([^'"]+)['"]/g);
    scan(/require\(\s*['"]([^'"]+)['"]\s*\)/g);
  }

  return found
    .map(({ path, index }) => ({ path, line: text.slice(0, index).split('\n').length }))
    .sort((a, b) => a.line - b.line);
}

// Rutas importadas en el contenido, según la sintaxis del lenguaje.
export const extractImports = (content, ext) => importsWithLines(content, ext).map((i) => i.path);
...
// Encuentra las violaciones de frontera en la LISTA de archivos dada (rutas relativas con '/').
// Es el núcleo del linter: `lintBoundaries` recorre el proyecto entero y el portón de calidad pasa
// solo los archivos cambiados de la tarea. Una sola implementación para los dos usos.
// Devuelve [{ file, line, from, to, import, kind: 'layer' | 'feature' }].
export async function lintBoundariesIn(projectDir, files) {
  const violations = [];
  for (const relPath of files) {
    ...
    for (const { path: importPath, line } of importsWithLines(content, ext)) {
      ...
      if ((FORBIDDEN_IMPORTS[source.layer] || []).includes(target.layer)) {
        violations.push({ file: relPath, line, from: source.layer, to: target.layer, import: importPath, kind: 'layer' });
      } else if (source.feature && target.feature && source.layer === target.layer && source.feature !== target.feature) {
        violations.push({ file: relPath, line, from: `${source.layer}/${source.feature}`, to: `${target.layer}/${target.feature}`, import: importPath, kind: 'feature' });
      }
    }
  }
  return violations;
...
// Recorre el proyecto entero y encuentra sus violaciones de frontera — `chalc verify`.
export async function lintBoundaries(projectDir) {
  return lintBoundariesIn(projectDir, await sourceFiles(projectDir));
}

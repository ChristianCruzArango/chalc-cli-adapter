...
import { logAiCall } from './tokenlog.mjs';
...
// Nombre de modelo estilo Ollama (`gpt-oss:20b`, `qwen3-coder:30b`): etiqueta local, nunca un ID de nube
// (OpenRouter usa `vendor/modelo`, OpenAI/Anthropic/Google no llevan ':').
const LOCAL_TAG = /^[^/\s]+:[^/\s]+$/;

/**
 * ¿Este modelo quedó fijado para OTRO proveedor? Pasa al cambiar el proveedor base (p. ej. ollama →
 * openrouter): los modelos por tarea y los roles del cli siguen guardados con nombres del anterior y
 * la API responde 400 "is not a valid model ID". `stampedFor` es el proveedor con el que se guardaron
 * (cfg.modelsFor / cfg.cli.rolesFor); si no hay sello — configs antiguas — se detecta por el nombre.
 */
export function isStaleModel(cfg, model, stampedFor) {
  const prov = PROVIDERS[cfg?.provider];
  if (!model || !prov?.needsKey) return false;   // local: manda lo que el usuario tenga instalado
  if (stampedFor) return stampedFor !== cfg.provider;
  return LOCAL_TAG.test(model);
}

/** Los modelos por tarea que SÍ sirven con el proveedor actual (los de otro proveedor se descartan). */
export function pruneStaleModels(cfg) {
  const models = { ...(cfg?.models || {}) };
  for (const task of Object.keys(models)) {
    if (isStaleModel(cfg, models[task], cfg?.modelsFor)) delete models[task];
  }
  return models;
}
...
export function modelForTask(cfg, task = 'default') {
  const prov = PROVIDERS[cfg?.provider];
  const picked = cfg?.models?.[task];
  const usable = picked && !isStaleModel(cfg, picked, cfg?.modelsFor) ? picked : '';
  return usable || cfg?.model || prov?.defaultModel || '';
}

// Además del modelo, etiqueta la config con la tarea: así chat() registra en el histórico de
// consumo QUÉ flujo gastó (spec/qa/repair…) sin adivinar nada (specs/002-chalc-tokens, R1).
export function configForTask(cfg, task = 'default') {
  return { ...(cfg || {}), model: modelForTask(cfg, task), task };
}
...
export function applyProfileModels(cfg, profile) {
  ...
  // los modelos fijados para OTRO proveedor se descartan: el perfil (o el modelo base) los reemplaza
  const models = pruneStaleModels(cfg);
  ...
  return { ...cfg, profile: profile.id, models, modelsFor: cfg.provider };
}
...
export async function chat(cfg, { system, user, maxTokens = 8192, json = false }) {
  ...
  if (prov.kind === 'anthropic') {
    ...
    const j = await res.json();
    recordUsage(j.usage);
    logAiCall({ provider: cfg.provider, model, task: cfg.task }, j.usage);   // histórico por proyecto (R1)

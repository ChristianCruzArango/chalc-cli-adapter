const BOOLEAN_FLAGS = new Set(['dry-run', 'yes', 'y', 'force', 'allow-exec', 'allow-login', 'allow-external-login', 'plan', 'up', 'agent', 'ccr', 'repair-plan', 'rerun', 'doctor', 'verify', 'ai', 'strict', 'branch', 'full', 'json', 'check', 'screenshots', 'worktree', 'terminals', 'console', 'watch']);
const VALUE_FLAGS = new Set(['target', 'method', 'mode', 'stack', 'name', 'lang', 'doc', 'azure', 'pat', 'jira', 'email', 'token', 'url', 'feature', 'spec', 'env', 'surface', 'max-steps', 'profile', 'spec-model', 'qa-model', 'repair-model', 'architecture', 'description', 'dir', 'back', 'movil', 'mobile', 'path', 'auth-token', 'workspace-dir', 'port']);
...
export function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (!VALUE_FLAGS.has(key) && !key.startsWith('auth-')) throw new Error(`Flag desconocida: --${key}`);
    if (eq !== -1) {

...
export async function configureAi(prompter) {
  ...
  // que llevan su propia key, no se tocan — siguen siendo válidos).
  const rebased = [];
  if (providerChanged && merged.cli?.roles) {
    const roles = { ...merged.cli.roles };
    for (const [role, v] of Object.entries(roles)) {
      if (typeof v === 'string' && v !== out.model) { roles[role] = out.model; rebased.push(role); }
    }
  ...
  if (providerChanged) console.log(c.yellow('  ! ' + t('aiProviderChanged', cfg.provider, provider, out.model)));

...
import { closingCycleMulti, gatePathOf } from '../gatehandoff.mjs';
...
export function featureHandoff({ backPath, backRel, frontPath, frontRel, mobilePath = '', mobileRel = '', branch, backBranchCreated, frontBranchCreated, mobileBranchCreated = false, specLang }) {
  ...
  // El ciclo de cierre, con el portón de CADA lado: el del back mide el contrato que expone y el del
  // front el que consume, así que cerrar uno no dice nada del otro (spec 007, R13).
  const cycle = closingCycleMulti({
    en,
    repos: [
      { label: en ? 'BACKEND' : 'BACKEND', gatePath: gatePathOf(backPath) },
      { label: en ? 'FRONTEND' : 'FRONTEND', gatePath: gatePathOf(frontPath) },
      ...(hasMobile ? [{ label: en ? 'MOBILE' : 'MÓVIL', gatePath: gatePathOf(mobilePath) }] : [])
    ]
  }).join('\n');
  if (en) {
    const repos = [
...
async function runFeatureWorktree({ cfg, prompter, backPath, frontPath, mobilePath, backStack, frontStack, mobileStack }) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    for (const side of plan.sides) {
      ...
      // Skills y portón DENTRO del worktree (spec 005 R9, spec 007 R15): el repo principal no se toca.

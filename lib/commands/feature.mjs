...
export function featureHandoff({ backPath, backRel, frontPath, frontRel, mobilePath = '', mobileRel = '', branch, backBranchCreated, frontBranchCreated, mobileBranchCreated = false, specLang }) {
  ...
  const cycle = advisorCycleMulti({
    ...
    repos: [
      ...
      ...(hasMobile ? [{ label: en ? 'MOBILE' : 'MÓVIL', nextPath: nextPathOf(mobilePath) }] : [])
    ]
  }).join('\n');
  if (en) {
    const repos = [
...
async function runFeatureWorktree({ cfg, prompter, backPath, frontPath, mobilePath, backStack, frontStack, mobileStack }) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    for (const side of plan.sides) {
      ...
      // La vista del workspace que tiene ESTE lado (spec 010, R2): quién posee el contrato, dónde
      // están los hermanos y dónde el buzón. Solo aquí se sabe — mirando un repo suelto es
      // indetectable, y por eso la sección viene apagada de la detección.

...
// El árbol se copia TAL CUAL bajo `.chalc/gate/`, así las rutas relativas entre sus módulos son las
// mismas en el catálogo y en el repo destino. La entrada a mano para el humano, `.chalc/gate.mjs`,
// es un lanzador de una línea: mover el archivo real un nivel arriba le rompería sus propios imports.
const destinationFor = (relFromSource) => `gate/${relFromSource}`;

const LAUNCHER = [
  '// Generado por chalc — no edites este archivo: se regenera al equipar el repo.',
  '// Portón de calidad: corre las etapas y escribe la evidencia en .chalc/gate.md.',
  "import './gate/gate.mjs';",
  ''
].join('\n');
...
export async function emitGate(projectPath, { role = '', language = '', sourceDir = GATE_DIR } = {}) {
  ...
  // El código del portón SIEMPRE se regenera: es artefacto de chalc, no superficie de configuración.
  for (const rel of await filesOf(sourceDir)) {
    written.push(await copyInto(projectPath, join(sourceDir, rel), destinationFor(rel)));
  }
  await writeFile(join(projectPath, GATE_REL), LAUNCHER, 'utf8');
  written.push(GATE_REL);

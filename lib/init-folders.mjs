...
const FLOW = {
  es: {
    ...
    layered: 'El flujo de dependencias apunta al dominio: `presentation` → `application` → `domain` ← `infrastructure`. El **dominio no depende de ninguna capa**; la infraestructura implementa sus interfaces (puertos). Nunca al revés.',
    ...
    addLayered: ['Modela la regla en `domain` (entidad + interfaz del repositorio/puerto).', 'Escribe el caso de uso en `application` usando esa interfaz.', 'Implementa la interfaz en `infrastructure` (HTTP/persistencia).', 'Crea la UI en `presentation` que invoca el caso de uso.', 'Test-First en cada paso; mutation testing al final (≥ 80%).'],
  ...
  en: {
    ...
    layered: 'Dependencies point inward to the domain: `presentation` → `application` → `domain` ← `infrastructure`. The **domain depends on no layer**; infrastructure implements its interfaces (ports). Never the other way around.',
    ...
    addLayered: ['Model the rule in `domain` (entity + repository/port interface).', 'Write the use case in `application` using that interface.', 'Implement the interface in `infrastructure` (HTTP/persistence).', 'Build the UI in `presentation` that calls the use case.', 'Test-First at each step; mutation testing at the end (≥ 80%).'],
...
// ¿Arquitectura por capas? (tiene domain + application). Si no, es feature-first.
function isLayered(folders) {
  return folders.some((x) => folderRole(x) === 'domain') && folders.some((x) => folderRole(x) === 'application');
}

...
export const MANDATORY_DESIGN_PRINCIPLES = lang === 'en'
  ? ['Clean Code', 'SOLID', 'modular architecture', 'high cohesion and low coupling', 'testable-by-design code', 'explicit responsibilities per folder, file and symbol']
  : ['Clean Code', 'SOLID', 'arquitectura modular', 'alta cohesión y bajo acoplamiento', 'código testeable desde el diseño', 'responsabilidades explícitas por carpeta, archivo y símbolo'];
...
export const STACKS = {
  angular: {
    ...
    scaffold: () => [{ label: 'ng new (Angular CLI)', command: 'npx', args: (name) => ['--yes', '@angular/cli@latest', 'new', name, '--routing', '--style=css', '--ssr=false', '--skip-git', '--defaults'], cwd: 'parent' }],
    verify: [
  ...
  nestjs: {
    ...
    scaffold: () => [{ label: 'nest new (NestJS CLI)', command: 'npx', args: (name) => ['--yes', '@nestjs/cli@latest', 'new', name, '--skip-git', '--package-manager', 'npm'], cwd: 'parent' }],
    verify: [
      { label: { es: 'instalar dependencias', en: 'install dependencies' }, command: 'npm', args: ['install'] },
      { label: { es: 'compilar', en: 'build' }, command: 'npm', args: ['run', 'build'] }
    ],

...
import { getStack, suggestArchitectures, analyzeProjectProposal } from './init.mjs';
...
function renderArchitectures(stackId) {
  return getStack(stackId).architectures
    .map((a) => `- ${a.id}: ${a.label} — ${a.fit} (tradeoff: ${a.tradeoff})`)
    .join('\n');
}
  ...
  for (let step = 1; step <= maxSteps; step++) {
    ...
    if (msg.done) {
      ...
      return finish(msg.architectureId, {
        ...
        clarifications: Array.isArray(msg.clarifications) ? msg.clarifications.map(String) : [],
        source: 'ai',

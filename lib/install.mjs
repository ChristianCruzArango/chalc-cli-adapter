...
import { procOpts, IS_WINDOWS } from './proc.mjs';
...
async function gitFetch(url, log) {
  ...
  if (ref && ref.startsWith('-')) throw new Error(`Ref de git no permitido: "${ref}"`);
  if (subpath && (subpath.startsWith('-') || subpath.split(/[/\\]/).includes('..'))) {
    throw new Error(`Subruta no permitida: "${subpath}"`);
  }
  ...
  // El destino vendorizado se confina a tmp aunque subpath sea absoluto/raro.
  const dir = subpath ? resolve(tmp, subpath) : tmp;
  if (!resolve(dir).startsWith(resolve(tmp))) throw new Error(`Subruta fuera del repo: "${subpath}"`);
  return { dir, cleanup: tmp };
}
...
async function skillsShFetch(name, log) {
  ...
  // En Windows npx es un shim .cmd que exige shell:true (lib/proc.mjs); con shell los args
  // pasan por cmd.exe, así que se rechazan metacaracteres antes de ejecutar.
  if (IS_WINDOWS) {
    const bad = args.find((a) => /[\s&|<>^%"']/.test(a));
    if (bad) throw new Error(`Argumento con caracteres no seguros para el shell de Windows: "${bad}"`);
  }
  log(`npx ${args.join(' ')} …`);
  execFileSync('npx', ['--yes', ...args], procOpts({ cwd: tmp, stdio: 'inherit' }));
  const dirs = [];
...
export async function fetchSkillSource(source, { log = () => {} } = {}) {
  ...
  if (kind === 'git') {
    ...
    return { kind, dirs: await findSkillDirs(r.dir), cleanup: r.cleanup };
  }
  if (kind === 'local') {
...
export async function installSkill({ source, CATALOG, prompter, force = false, allowExternalExec = false, log = () => {} }) {
  ...
  const { dirs, cleanup } = await fetchSkillSource(source, { log });

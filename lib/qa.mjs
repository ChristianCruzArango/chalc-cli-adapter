...
export async function readSpecContext(projectPath, specId) {
  ...
  if (!files['spec.md']) throw new Error(`No existe la spec: specs/${specId}/spec.md`);
...
export function selectEnvironment(options, query) {
  ...
  if (!found) throw new Error(`Entorno "${query}" no detectado. Opciones: ${(options || []).map(environmentKey).join(', ') || 'ninguna'}`);
  return found;
}
...
export function buildBrowserTests(context, inputs = {}) {
  ...
  for (const item of requirements(context.files['spec.md'])) {
    ...
    if (!detail.path || !detail.expected) {
      lines.push(`test.fixme('${title}', 'Faltan ruta o resultado observable en specs/${context.id}/qa/inputs.json.');`, '');
      continue;
    }
    lines.push(
...
export function buildStartCommand(env, { port } = {}) {
  if (!env) throw new Error('No hay entorno seleccionado para levantar.');
  if (env.type === 'compose') {
  ...
  if (!match) throw new Error(`No sé cómo levantar el entorno: ${env.label || env.command || '(desconocido)'}`);
...
export function normalizeQaUrl(raw) {
  let url;
  try { url = new URL(String(raw || '')); } catch { throw new Error('La URL QA debe ser http(s) válida.'); }
  if (!['http:', 'https:'].includes(url.protocol)) throw new Error('La URL QA debe usar http o https.');
...
async function command(command, args, cwd) {
  try {
    const { stdout } = await execFileAsync(command, args, { timeout: 8_000, cwd });
    return { ok: true, stdout: String(stdout).trim() };
  } catch (error) {
    return { ok: false, error: error?.code === 'ENOENT' ? 'not-found' : String(error?.stderr || error?.message || '').trim() };
  }
}

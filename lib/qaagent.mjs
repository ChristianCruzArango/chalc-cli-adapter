...
import { redactObservation as redactObservationValue, redactSensitiveText } from './redact.mjs';
...
export async function runQaAgent(opts) {
  ...
  for (let step = 1; step <= maxSteps; step++) {
    ...
    const observation = redactObservation(await executor(message.action));
  ...
  const exec = async (action) => {
    ...
    try {
      for (const stp of action.steps || []) {
        if (stp.op === 'goto') {
        ...
        else if (stp.op === 'expectUrl') { const found = new RegExp(stp.value).test(page.url()); log.push(`expectUrl ${stp.value} ⇒ ${found ? 'sí' : 'no'}`); if (!found) return redactObservation({ ok: false, log, detail: `URL actual: ${page.url()}` }); }
        else if (stp.op === 'inspect') { const text = await page.locator('body').innerText(); return redactObservation({ ok: true, log, url: page.url(), text }); }
          ...
          if (!found) return redactObservation({ ok: false, log, detail: `no se encontró "${stp.value}" en ${stp.selector}` });

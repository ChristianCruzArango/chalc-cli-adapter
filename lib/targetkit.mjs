...
const MANDATORY_SKILLS = ['clean-code', 'solid-principles', 'modular-architecture'];
...
export function mandatoryPrinciplesBlock(skills = []) {
  ...
  return lang === 'en'
    ? [
        ...
        'Clean Code, SOLID and **modular architecture** apply to **all** code in this project — no exceptions:',
        'high cohesion and low coupling, small units, explicit names, one responsibility per file/folder/symbol,',
        'and code that is testable by design. The `clean-code`, `solid-principles` and `modular-architecture`',
        'skills define the detail — **open and apply them by default**, not only when explicitly asked.'
      ].join('\n')
    : [
        '### ✅ Principios obligatorios (SIEMPRE)',
        'Clean Code, SOLID y **arquitectura modular** aplican a **todo** el código de este proyecto, sin excepción:',
        'alta cohesión y bajo acoplamiento, unidades pequeñas, nombres explícitos, una responsabilidad por',
        'archivo/carpeta/símbolo, y código testeable desde el diseño. Las skills `clean-code`, `solid-principles`',
        'y `modular-architecture` definen el detalle — **ábrelas y aplícalas por defecto**, no solo cuando se pidan.'
      ].join('\n');
}
